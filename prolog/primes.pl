...
sieve(_, _, SQUARE, LIMIT) :- SQUARE > LIMIT, !.
sieve(N, CUR, _, LIMIT) :-
    composite(CUR), !,
    NEXT is CUR + 2, sieve(N, NEXT, LIMIT).
sieve(N, CUR, SQUARE, LIMIT) :-
    assert(table_nth(N, CUR)),
    mark_comp(CUR, SQUARE, LIMIT), !,
    NEXT is CUR + 2, N1 is N + 1, sieve(N1, NEXT, LIMIT).

divisor(N, N) :- prime(N), !.

mark_comp(P, _, _) :- composite(P), !.
mark_comp(P, SQUARE, LIMIT) :- DIFF is P + P, mark_comp(P, DIFF, SQUARE, LIMIT).
mark_comp(_, _, CUR, LIMIT) :- CUR > LIMIT, !.
mark_comp(P, DIFF, CUR, LIMIT) :-
    assert(composite(CUR)), assert(divisor(CUR, P)),
    NEXT is DIFF + CUR, mark_comp(P, DIFF, NEXT, LIMIT).

prime_divisors(1, []) :- !.
prime_divisors(N, [H | T]) :-
    number(N), divisor(N, H),
    N1 is div(N, H), prime_divisors(N1, T), !.
prime_divisors(N, [H | T]) :- prime_factors_list(N, [H | T], H), !.
prime_factors_list(1, [], _) :- !.
prime_factors_list(N, [H | T], MIN) :-
    H >= MIN, prime(H),
    prime_factors_list(N1, T, H), N is N1 * H, !.
